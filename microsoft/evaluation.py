...
from microsoft.pandas_df_utils import (
    has_columns,
    has_same_base_dtype,
    lru_cache_df,
)
...
def ndcg_at_k(
    ...
    **kwargs
    ...
    df_user = df_dcg.groupby(col_user, as_index=False,
                             sort=False).agg({"dcg": "sum"})
...
def get_top_k_items(
    ...
    if k is None:
        ...
        top_k_items = (
            dataframe.sort_values([col_user, col_rating],
    ...
    # Add ranks
    top_k_items["rank"] = top_k_items.groupby(
        col_user, sort=False).cumcount() + 1
    return top_k_items
...
def _get_pairwise_items(
    ...
    df_user_i1_i2 = pd.merge(df_user_i1, df_user_i2,
                             how="inner", on=[col_user])
...
def novelty(train_df, reco_df, col_user=DEFAULT_USER_COL, col_item=DEFAULT_ITEM_COL):
    ...
    avg_novelty = reco_item_novelty.agg({"product": "sum"})[
        0] / n_recommendations
