...
from microsoft.download_utils import (
    maybe_download,
    download_path,
    unzip_file,
)
...
def get_valid_input(session, valid_file_path):
    ...
    if os.path.isfile(valid_file_path):
        logger.info(
            f"Validation file {valid_file_path} successfully generated")
    else:
...
def get_user_history(train_history, valid_history, user_history_path):
    ...
    if os.path.isfile(user_history_path):
        logger.info(
            f"User history file {user_history_path} successfully generated")
    else:
